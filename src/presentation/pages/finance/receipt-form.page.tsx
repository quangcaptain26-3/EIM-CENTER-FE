...
const ALLOWED_ENROLL = [ENROLLMENT_STATUS.trial, ENROLLMENT_STATUS.active, ENROLLMENT_STATUS.paused] as const;
...
export default function ReceiptFormPage() {
  ...
  const onValid = async (v: FormValues) => {
    ...
    try {
      ...
      const id = parseCreatedId(payload);
          ...
          {studentId ? (
            <div>
              ...
              {loadEnr ? (
                ...
                <p className="text-sm text-amber-400/90">Không có ghi danh phù hợp (trial / active / paused).</p>
              ) : (
                      ...
                      <li key={e.id}>
                        ...
                        >
                          ...
                          <p className="mt-1 text-xs text-[var(--text-muted)]">
                            ...
                            {debt != null ? (
                              <span className="text-[var(--text-secondary)]"> · Công nợ: {formatVnd(debt)}</span>
                            ) : (
                              <span className="text-[var(--text-muted)]"> · Công nợ: …</span>
                            )}
                          </p>

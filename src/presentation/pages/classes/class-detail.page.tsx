...
import { useClass, useClassAttendanceMatrix, useClassRoster } from '@/presentation/hooks/classes/use-classes';
...
import { ClassAttendancePivot } from '@/presentation/components/classes/class-attendance-pivot';
...
type TabId = 'sessions' | 'roster' | 'attendance' | 'staff';
...
export default function ClassDetailPage() {
  ...
  const { matrix: attendanceMatrix, isLoading: matrixLoading } = useClassAttendanceMatrix(
    tab === 'attendance' ? classId : undefined,
  );
    ...
    () => [
      ...
      rosterColumnHelper.display({
        ...
        cell: (ctx) => {
          ...
          if (d > 0) {
            return <span className="text-red-400">Cần đóng thêm {fmt.currencyShort(d)}</span>;
          }
          return '—';
        },
      }),
      ...
      {tab === 'roster' ? (
        <div className="space-y-3">
          ...
          <DataTable
            ...
            emptyMessage="Chưa có học viên nào trong lớp này"

...
import {
  ...
  getPayroll,
  getPayrolls,
  getUnfinalizedPayrollSummaries,
  previewPayroll,
...
export interface UnfinalizedPayrollsParams {
  month: number;
  year: number;
  page: number;
  limit: number;
...
export function useUnfinalizedPayrolls(params: UnfinalizedPayrollsParams) {
  const apiParams = {
    month: params.month,
    year: params.year,
    page: params.page,
  ...
  const q = useQuery({
    queryKey: QUERY_KEYS.PAYROLL.unfinalized(apiParams),
    queryFn: () => getUnfinalizedPayrollSummaries(apiParams),
    staleTime: STALE_PAYROLL_MS,
  });
  const parsed = q.data ? parsePayrollListResponse(q.data) : { items: [], total: 0, page: 1, limit: 20 };
  return {

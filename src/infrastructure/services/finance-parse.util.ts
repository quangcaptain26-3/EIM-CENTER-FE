...
export function parsePaymentStatusListResponse(raw: unknown): {
  ...
  const items: PaymentStatusRow[] = arr.map((row) => {
    ...
    return {
      ...
      parentZalo: str(r.parentZalo ?? r.parent_zalo) || undefined,
      ...
      enrolledAt: r.enrolledAt ? String(r.enrolledAt) : r.enrolled_at ? String(r.enrolled_at) : undefined,
      tuitionFee: num(r.tuitionFee ?? r.tuition_fee),
      totalPaid: num(r.totalPaid ?? r.total_paid),
      debt: num(r.debt),
      isDebtOver30Days: Boolean(r.isDebtOver30Days ?? r.is_debt_over_30_days),
    };
  });
  return {
    items,
...
function parsePayrollEntity(o: Record<string, unknown>): PayrollListRow {
  const isPending = Boolean(o.isPending ?? o.is_pending);
  return {
    id: str(o.id),

...
export interface UnfinalizedPayrollListParams {
  month: number;
  year: number;
  teacherId?: string;
  page?: number;
...
/** GV có buổi trong kỳ nhưng chưa chốt lương — cần month+year. */
export async function getUnfinalizedPayrollSummaries(
  params: UnfinalizedPayrollListParams,
): Promise<PagedResponse<PayrollRecord>> {
  const res = await apiClient.get('/payroll/unfinalized-summaries', {
    params: compactParams(params as unknown as Record<string, unknown>),
  });
  return unwrapApiData<PagedResponse<PayrollRecord>>(res);
}
...
export async function approveRefund(id: string, note?: string): Promise<void> {
  await apiClient.patch(`/refund-requests/${id}/approve`, note !== undefined ? { reviewNote: note } : {});
}
